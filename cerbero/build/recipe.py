...
class Recipe(FilesProvider, metaclass=MetaRecipe):
    ...
    def _write_gst_la_file(self, la_path, pcname, major, minor, micro, env):
        ...
        for lib in set(ret.split()):
            lafile = os.path.join(self.config.libdir, lib[2:] + '.la')
    ...
    def generate_gst_la_files(self):
        ...
        env = os.environ.copy()
        env['PKG_CONFIG_LIBDIR'] += ':' + pluginpcdir
        if self.use_system_libs:
            add_system_libs(self.config, env)
        # Get la file -> pkg-config name mappping
        libs_la_files = {}
        plugin_la_files = {}
        for f in self.devel_files_list():
            if not f.endswith('.a') or not f.startswith('lib/'):
                continue
            if not os.path.isfile(os.path.join(self.config.prefix, f)):
                m.warning('Plugin {!r} not found'.format(f))
                continue
